...
import { User, Users, Workflow, Tag, Receipt, Shield, ChevronRight } from "lucide-react";
...
export default function ClientSettingsPage() {
    ...
    return (
        <div className="min-h-screen bg-neutral-50 p-6">
            <Breadcrumb items={[{ label: "Flow" }, { label: "Client" }, { label: "Settings" }]} />
                ...
                <div className="space-y-8 w-full animate-in fade-in duration-500">
                    <div className="space-y-4">
                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                            <div>
                                ...
                                <p className="text-sm text-neutral-500 mt-1">Configure client management settings.</p>
                            </div>
                    ...
                    {viewMode === "team" ? (
                        <div className="grid gap-4">
                            <SettingCard icon={<Tag className="w-5 h-5" />} title="Client Categories" description="Manage client types and segments" />
                            <SettingCard icon={<Workflow className="w-5 h-5" />} title="Contract Workflow" description="Configure contract approval process" />
                            <SettingCard icon={<Receipt className="w-5 h-5" />} title="Invoice Settings" description="Billing templates and payment terms" />
                            <SettingCard icon={<Shield className="w-5 h-5" />} title="Permissions" description="Role-based access control" />
                        </div>
                    ) : (
                        <div className="grid gap-4">
                            <SettingCard icon={<Tag className="w-5 h-5" />} title="My Notifications" description="Client updates and reminders" />
