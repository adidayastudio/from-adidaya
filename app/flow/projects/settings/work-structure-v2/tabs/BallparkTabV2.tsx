...
export default function BallparkTabV2({ workspaceId, projectTypeId }: Props) {
    ...
    const handleAddRowOption = async (targetItem: WBSItemLocal, position: "above" | "below" | "subtask") => {
        ...
        if (position === "subtask") {
            ...
            // Check if target code is numeric at the end
            ...
            // Shifting sort_orders (and codes only if numeric) in parallel
            const promises = [];
            for (let i = siblings.length - 1; i >= insertIdx; i--) {
                ...
                // Only shift WBS code string if it ends with a numeric index
                if (isNumeric) {
            ...
            if (isNumeric) {
                ...
                // If L1 is code 'S', sibling added below can be 'NEW_DISCIPLINE' or similar
                newCode = "NEW";
            }
