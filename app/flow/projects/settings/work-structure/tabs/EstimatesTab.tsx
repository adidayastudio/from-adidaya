...
// Local WBS Item type (Compatible with WBSList)
type WBSItemLocal = {
    id: string;
    code: string;
    nameEn: string;
...
export default function EstimatesTab({ workspaceId, projectTypeId, headerContent }: Props) {
    ...
    const loadData = useCallback(async () => {
        ...
        if (data) {
            // Build Tree
            // Map DB fields to Local fields
            const nodes = data.map((d: any) => ({
                id: d.id,
                code: d.code,
                nameEn: d.name, // EN Name
                nameId: d.description, // ID Name stored in description
                level: d.level,
    ...
    // --- View Logic ---
    // Rules: Summary = L1-L2, Breakdown = L1-L3 (Estimates View)
    // Actually, user might want to see deeper? The mock logic cut off at L3.
    // Let's stick to user preference (Summary=2 levels, Breakdown=All/3?)
    const visibleTree = useMemo(() => {
        const maxIndent = viewMode === "SUMMARY" ? 1 : 10; // 0=L1, 1=L2
    ...
    // 3. ADD CHILD
    const handleAddChild = useCallback(async (parentId: string, parentLevel: number) => { // parentLevel is 0-indexed relative to logic?
        // Note: 'level' arg here comes from WBSList, usually indent level.

        // Find parent to get indent_level and code
        // For simplicity, we fetch fresh parent or find in tree?
        // Let's find in tree.
        ...
        const newItem = {
            ...
            project_type_id: projectTypeId, // Assign type?
            parent_id: parentId,
            code: newCode,
            name: "New Work Item",
            description: "Pekerjaan Baru",
            indent_level: newIndent,
            level: `L${newIndent + 1}`, // Roughly L1, L2..
            sort_order: nextNum
        };
        ...
        // DB Insert
        const { data, error } = await supabase.from('work_breakdown_structure').insert(newItem).select().single();
        if (error) {
            console.error("Failed to add item:", error);
            // In a real app, revert optimistic update here
            loadData(); // Sync fallback
        }
    ...
    const handleAddSibling = useCallback(async (siblingId: string, position: "above" | "below") => {
        ...
        // Just mocking simpler logic: Append to parent of sibling (below)
        // Implementing 'above' needs reordering which is complex.
        // Let's implement 'append to same parent' for now to unblock 'Add'
        ...
        const parentId = parent ? parent.id : null; // Root if null

        // Calculate New Defaults
        // If Root, get max sort of root. If child, get max sort of children.
        // For simplicity, just append to end. 'position' is harder without full reorder logic.
        ...
        const newCode = parentId ? `${prefix}.${nextSuffix}` : `${nextSuffix}`; // Rough Code Logic

        const newItem = {
            id: crypto.randomUUID(),
            workspace_id: workspaceId,
            project_type_id: projectTypeId,
            parent_id: parentId, // Can be null for Roots
            code: newCode + (position === 'above' ? '-temp' : ''), // Todo: cleanup code logic
            name: "New Sibling Item",
            description: "Pekerjaan Baru",
            indent_level: newIndent,
            level: `L${newIndent + 1}`,
            sort_order: 999 // Put at end for now
        };

        // DB Insert
        const { error } = await supabase.from('work_breakdown_structure').insert(newItem);
        if (!error) loadData(); // Reload to sort correctly

    }, [wbsItems, workspaceId, projectTypeId, loadData]);

    // 5. REORDER (Stub for now, complicated)
    const handleReorder = useCallback((parentId: string | null, fromIndex: number, toIndex: number) => {
        // Drag and drop not fully connected to DB sorting yet.
    ...
    return (
        <div className="space-y-6">
            ...
            {loading ? (
                <div className="h-64 flex items-center justify-center text-neutral-400 text-sm">Loading WBS...</div>
            ) : (
