...
import { User, Users, Workflow, Tag, Truck, Shield, ChevronRight } from "lucide-react";
...
export default function SupplySettingsPage() {
    ...
    return (
        <div className="min-h-screen bg-neutral-50 p-6">
            <Breadcrumb items={[{ label: "Flow" }, { label: "Supply" }, { label: "Settings" }]} />
                ...
                <div className="space-y-8 w-full animate-in fade-in duration-500">
                    <div className="space-y-4">
                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                            <div>
                                ...
                                <p className="text-sm text-neutral-500 mt-1">Configure supply chain settings.</p>
                            </div>
                    ...
                    {viewMode === "team" ? (
                        <div className="grid gap-4">
                            <SettingCard icon={<Workflow className="w-5 h-5" />} title="Approval Flow" description="Configure PO approval workflow" />
                            <SettingCard icon={<Tag className="w-5 h-5" />} title="Item Categories" description="Manage supply categories" />
                            <SettingCard icon={<Truck className="w-5 h-5" />} title="Delivery Settings" description="Configure delivery tracking" />
                            <SettingCard icon={<Shield className="w-5 h-5" />} title="Permissions" description="Role-based access control" />
                        </div>
                    ) : (
                        <div className="grid gap-4">
                            <SettingCard icon={<Tag className="w-5 h-5" />} title="My Preferences" description="Default request settings" />
