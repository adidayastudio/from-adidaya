...
import { User, Users, Workflow, Tag, ClipboardCheck, Shield, ChevronRight } from "lucide-react";
...
export default function TrackSettingsPage() {
    ...
    return (
        <div className="min-h-screen bg-neutral-50 p-6">
            <Breadcrumb items={[{ label: "Flow" }, { label: "Track" }, { label: "Settings" }]} />
                ...
                <div className="space-y-8 w-full animate-in fade-in duration-500">
                    <div className="space-y-4">
                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                            <div>
                                ...
                                <p className="text-sm text-neutral-500 mt-1">Configure project tracking settings.</p>
                            </div>
                    ...
                    {viewMode === "team" ? (
                        <div className="grid gap-4">
                            <SettingCard icon={<Tag className="w-5 h-5" />} title="Issue Categories" description="Manage issue types and priorities" />
                            <SettingCard icon={<ClipboardCheck className="w-5 h-5" />} title="Inspection Types" description="Configure inspection checklists" />
                            <SettingCard icon={<Workflow className="w-5 h-5" />} title="Progress Phases" description="Define project phases and milestones" />
                            <SettingCard icon={<Shield className="w-5 h-5" />} title="Permissions" description="Role-based access control" />
                        </div>
                    ) : (
                        <div className="grid gap-4">
                            <SettingCard icon={<Tag className="w-5 h-5" />} title="My Notifications" description="Progress and issue alerts" />
