...
export function ClockViewToggleUI({
    viewMode,
    setViewMode,
    canAccessTeam,
}: {
    ...
    return (
        <div className="relative hidden md:inline-flex p-1 items-center h-9 rounded-full bg-white/10 dark:bg-neutral-800/10 backdrop-blur-xl border border-white/20 dark:border-neutral-700/20 shadow-sm transition-all duration-300 pointer-events-auto">
            {/* PERSONAL BUTTON */}
            <button
...
export function ClockViewToggle() {
    ...
    if (isLoading) {
        return (
            <div className="hidden md:flex items-center gap-2 p-1 rounded-full h-9 bg-neutral-100/10 border border-white/10 animate-pulse w-[180px]" />
        );
    }
    return <ClockViewToggleUI viewMode={viewMode} setViewMode={setViewMode} canAccessTeam={canAccessTeam} />;
}
