...
import { cnTasks, CN_SECTIONS, getCNTasksBySection } from "./data/cn";
...
export default function StageCN() {
    const [openSection, setOpenSection] = useState<string | null>("CN-01");
    const [selectedTask, setSelectedTask] = useState<Task | null>(null);
    const [isDrawerOpen, setIsDrawerOpen] = useState(false);

    const [allTasks, setAllTasks] = useState<Record<string, Task[]>>(() => {
        const grouped: Record<string, Task[]> = {};
        CN_SECTIONS.forEach(section => {
            grouped[section.code] = getCNTasksBySection(section.code);
        });
        return grouped;
    });
    ...
    const handleAddTask = useCallback((sectionCode: string, parentId?: string, mode?: "above" | "below" | "subtask", relativeId?: string) => {
        setAllTasks(prev => {
            ...
            const sectionWeight = getSectionWeight(CN_SECTIONS, sectionCode);
    ...
    const renderSection = (code: string, title: string) => {
        ...
        const section = CN_SECTIONS.find(s => s.code === code);
    ...
    return (
        <>
            <div className="space-y-0 divide-y divide-neutral-100">
                {CN_SECTIONS.map(section => renderSection(section.code, section.title))}
            </div>
