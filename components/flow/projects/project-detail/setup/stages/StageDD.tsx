...
import { ddTasks, DD_SECTIONS, getDDTasksBySection } from "./data/dd";
...
export default function StageDD() {
    const [openSection, setOpenSection] = useState<string | null>("DD-01");
    ...
    const [allTasks, setAllTasks] = useState<Record<string, Task[]>>(() => {
        ...
        DD_SECTIONS.forEach(section => {
            grouped[section.code] = getDDTasksBySection(section.code);
        });
        return grouped;
    });
    ...
    const handleAddTask = useCallback((sectionCode: string, parentId?: string, mode?: "above" | "below" | "subtask", relativeId?: string) => {
        setAllTasks(prev => {
            ...
            const sectionWeight = getSectionWeight(DD_SECTIONS, sectionCode);
    ...
    const renderSection = (code: string, title: string) => {
        ...
        const section = DD_SECTIONS.find(s => s.code === code);
    ...
    return (
        <>
            <div className="space-y-0 divide-y divide-neutral-100">
                {DD_SECTIONS.map(section => renderSection(section.code, section.title))}
            </div>
