...
import { hoTasks, HO_SECTIONS, getHOTasksBySection } from "./data/ho";
...
export default function StageHO() {
    const [openSection, setOpenSection] = useState<string | null>("HO-01");
    const [selectedTask, setSelectedTask] = useState<Task | null>(null);
    const [isDrawerOpen, setIsDrawerOpen] = useState(false);

    const [allTasks, setAllTasks] = useState<Record<string, Task[]>>(() => {
        const grouped: Record<string, Task[]> = {};
        HO_SECTIONS.forEach(section => {
            grouped[section.code] = getHOTasksBySection(section.code);
        });
        return grouped;
    });
    ...
    const handleAddTask = useCallback((sectionCode: string, parentId?: string, mode?: "above" | "below" | "subtask", relativeId?: string) => {
        setAllTasks(prev => {
            ...
            const sectionWeight = getSectionWeight(HO_SECTIONS, sectionCode);
    ...
    const renderSection = (code: string, title: string) => {
        ...
        const section = HO_SECTIONS.find(s => s.code === code);
    ...
    return (
        <>
            <div className="space-y-0 divide-y divide-neutral-100">
                {HO_SECTIONS.map(section => renderSection(section.code, section.title))}
            </div>
