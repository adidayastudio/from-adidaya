...
import { koTasks, KO_SECTIONS, getKOTasksBySection } from "./data/ko";
...
/**
 * Renumber all tasks in a section sequentially.
 */
...
export default function StageKO() {
  const [openSection, setOpenSection] = useState<string | null>("KO-01");
  const [selectedTask, setSelectedTask] = useState<Task | null>(null);
  const [isDrawerOpen, setIsDrawerOpen] = useState(false);

  const [allTasks, setAllTasks] = useState<Record<string, Task[]>>(() => {
    const grouped: Record<string, Task[]> = {};
    KO_SECTIONS.forEach(section => {
      grouped[section.code] = getKOTasksBySection(section.code);
    });
    return grouped;
  });
  ...
  /* ======================
     CRUD HANDLERS with WEIGHT REDISTRIBUTION
  ====================== */
  ...
  const handleAddTask = useCallback((sectionCode: string, parentId?: string, mode?: "above" | "below" | "subtask", relativeId?: string) => {
    setAllTasks(prev => {
      ...
      const sectionWeight = getSectionWeight(KO_SECTIONS, sectionCode);
      ...
      // REDISTRIBUTE weights (new task weight = 0 triggers redistribution)
  ...
  const handleUpdateTask = useCallback((sectionCode: string, taskId: string, field: keyof Task, value: any) => {
    setAllTasks(prev => {
      ...
      if (field === "weight") {
        // Use weight edit handler for proportional redistribution
        const tasksWithNewWeight = sectionTasks.map(t =>
          t.id === taskId ? { ...t, weight: value } : t
        );
        updatedTasks = handleWeightEdit(tasksWithNewWeight, taskId, value, sectionWeight);
      } else {
        // Regular field update
        updatedTasks = sectionTasks.map(t =>
  ...
  const handleDeleteTask = useCallback((sectionCode: string, taskId: string) => {
    setAllTasks(prev => {
      ...
      // REDISTRIBUTE weights (give deleted task's weight back to others)
  ...
  const handleReorderTask = useCallback((sectionCode: string, fromIndex: number, toIndex: number) => {
    setAllTasks(prev => {
      ...
      sectionTasks = renumberTasks(sectionTasks, sectionCode);
      // Weights don't change on reorder, just codes
      return { ...prev, [sectionCode]: sectionTasks };
    });
  }, []);
  ...
  const renderSection = (code: string, title: string) => {
    ...
    const section = KO_SECTIONS.find(s => s.code === code);
  ...
  return (
    <>
      <div className="space-y-0 divide-y divide-neutral-100">
        {KO_SECTIONS.map(section => renderSection(section.code, section.title))}
      </div>
