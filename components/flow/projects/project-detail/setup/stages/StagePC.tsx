...
import { pcTasks, PC_SECTIONS, getPCTasksBySection } from "./data/pc";
...
export default function StagePC() {
    const [openSection, setOpenSection] = useState<string | null>("PC-01");
    const [selectedTask, setSelectedTask] = useState<Task | null>(null);
    const [isDrawerOpen, setIsDrawerOpen] = useState(false);

    const [allTasks, setAllTasks] = useState<Record<string, Task[]>>(() => {
        const grouped: Record<string, Task[]> = {};
        PC_SECTIONS.forEach(section => {
            grouped[section.code] = getPCTasksBySection(section.code);
        });
        return grouped;
    });
    ...
    const handleAddTask = useCallback((sectionCode: string, parentId?: string, mode?: "above" | "below" | "subtask", relativeId?: string) => {
        setAllTasks(prev => {
            ...
            const sectionWeight = getSectionWeight(PC_SECTIONS, sectionCode);
    ...
    const renderSection = (code: string, title: string) => {
        ...
        const section = PC_SECTIONS.find(s => s.code === code);
    ...
    return (
        <>
            <div className="space-y-0 divide-y divide-neutral-100">
                {PC_SECTIONS.map(section => renderSection(section.code, section.title))}
            </div>
