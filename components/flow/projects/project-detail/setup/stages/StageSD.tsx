...
import { sdTasks, SD_SECTIONS, getSDTasksBySection } from "./data/sd";
...
export default function StageSD() {
    const [openSection, setOpenSection] = useState<string | null>("SD-01");
    const [selectedTask, setSelectedTask] = useState<Task | null>(null);
    const [isDrawerOpen, setIsDrawerOpen] = useState(false);

    const [allTasks, setAllTasks] = useState<Record<string, Task[]>>(() => {
        const grouped: Record<string, Task[]> = {};
        SD_SECTIONS.forEach(section => {
            grouped[section.code] = getSDTasksBySection(section.code);
        });
        return grouped;
    });
    ...
    const handleAddTask = useCallback((sectionCode: string, parentId?: string, mode?: "above" | "below" | "subtask", relativeId?: string) => {
        setAllTasks(prev => {
            ...
            const sectionWeight = getSectionWeight(SD_SECTIONS, sectionCode);
    ...
    const renderSection = (code: string, title: string) => {
        ...
        const section = SD_SECTIONS.find(s => s.code === code);
    ...
    return (
        <>
            <div className="space-y-0 divide-y divide-neutral-100">
                {SD_SECTIONS.map(section => renderSection(section.code, section.title))}
            </div>
