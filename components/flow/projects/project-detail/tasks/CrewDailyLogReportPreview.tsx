...
export default function CrewDailyLogReportPreview({
  ...
  const [projectList, setProjectList] = useState<{ id: string; code: string; name: string }[]>([]);
  const [activeProject, setActiveProject] = useState<string>(projectName || "JPF - Masterplan & Architecture");
    ...
    async function loadProjects() {
      try {
        ...
        if (dbProjects && dbProjects.length > 0) {
          ...
          if (formatted[0]) {
            setActiveProject(formatted[0].name);
          }
        }
      } catch (err) {
        console.error("Using default project name:", err);
      }
    }
    loadProjects();
  }, []);
  ...
  const projInfo = (() => {
    const matched = projectList.find(
      (p) => p.name === activeProject || p.id === activeProject || p.code === activeProject
    );
    
    let code = "PROJ";
    let cleanName = activeProject || "Proyek";
    let location = "—";
    let stage = "—";

    if (matched) {
      if (matched.cleanCode) code = matched.cleanCode;
      if (matched.name) cleanName = matched.name;
      if (matched.location) location = formatLocationStr(matched.location);
      if (matched.stage && typeof matched.stage === "string") stage = matched.stage;
    ...
    return { code, cleanName, location: formatLocationStr(location), stage };
  })();
            ...
            <div className="relative inline-flex items-center">
              ...
              >
                {projectList.length > 0 ? (
                  projectList.map((p) => (
                    <option key={p.id} value={p.name} className="dark:bg-neutral-900">
