...
// 31 days in a month representation
type DayVal = "C" | "B" | "H" | "-";

interface HourlyMonthlyMatrixRow {
  hour: number;
  days: DayVal[]; // Array of 31 values for dates 1..31
  remarks?: string;
}
...
export default function MonthlyWeatherReportPreview({
  ...
  // Today is 17 Aug 2026 (Day 17 of the current month)
  const currentTodayDate = 17;

  // Number of days in selected month (e.g. Aug 2026 = 31 days)
  ...
  // Array of date numbers: [1, 2, 3, ..., 31]
  const dateNumbers = useMemo(() => {
    return Array.from({ length: daysInMonth }, (_, i) => i + 1);
  }, [daysInMonth]);

  // Generate initial 24h x 31d default matrix (Auto-synced from daily logs)
  // Dates 1..17 are logged. Dates 18..31 are FUTURE DATES -> DISABLED "-"
  const generateDefaultMatrix = (): HourlyMonthlyMatrixRow[] => {
    const defaultRows: HourlyMonthlyMatrixRow[] = [];
    for (let h = 1; h <= 24; h++) {
      const daysArr: DayVal[] = [];
      for (let d = 1; d <= 31; d++) {
        if (d > currentTodayDate) {
          // Future date -> Unlogged (-)
          daysArr.push("-");
        } else {
          // Logged dates 1..17
          let cond: "C" | "B" | "H" = "C";
          if (h <= 6) cond = "H";
          else if (h >= 7 && h <= 17) cond = "C";
          else if (h >= 18 && h <= 21) cond = "B";
          else cond = "H";
          daysArr.push(cond);
        }
      }
      ...
      defaultRows.push({
        hour: h,
        days: daysArr,
        remarks: h === 14 ? "Midday heat peak" : "",
      });
    }
    return defaultRows;
  };

  const [hourlyMatrix, setHourlyMatrix] = useState<HourlyMonthlyMatrixRow[]>(generateDefaultMatrix);

  const [monthlyNotes, setWeeklyNotes] = useState(
    "Monthly weather observation for August 2026 up to 17 Aug records 187 total sunny hours, 68 cloudy hours, and 149 rainy hours. Dates 18 to 31 August remain unlogged as future dates."
  );

  // Calculate totals per day and grand totals for month
  const monthTotals = useMemo(() => {
    const sunnyCounts = dateNumbers.map((d) => {
      if (d > currentTodayDate) return 0;
      return hourlyMatrix.filter((r) => r.days[d - 1] === "C").length;
    });

    const cloudyCounts = dateNumbers.map((d) => {
      if (d > currentTodayDate) return 0;
      return hourlyMatrix.filter((r) => r.days[d - 1] === "B").length;
    });

    const rainyCounts = dateNumbers.map((d) => {
      if (d > currentTodayDate) return 0;
      return hourlyMatrix.filter((r) => r.days[d - 1] === "H").length;
    });
  ...
  }, [hourlyMatrix, dateNumbers, currentTodayDate]);
  ...
  const handleCellChange = (hourIndex: number, dayNum: number, val: DayVal) => {
    setIsOverridden(true);
    setHourlyMatrix((prev) => {
      const copy = [...prev];
      const newDays = [...copy[hourIndex].days];
      newDays[dayNum - 1] = val;
      copy[hourIndex] = { ...copy[hourIndex], days: newDays };
      return copy;
    });
  };
  ...
  return (
    <div className="w-full space-y-4 font-sans">
      ...
      {activeTab === "edit" ? (
        /* EDIT DATA FORM FOR 31-DAY MONTHLY WEATHER MATRIX WITH DISABLED FUTURE DATES */
        <div className="space-y-6 animate-in fade-in duration-300 py-2 max-w-6xl mx-auto">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="space-y-0.5">
                ...
                <p className="text-blue-700 dark:text-blue-300 text-[11px]">
                  Header kolom hanya menampilkan angka tanggal (1 – {daysInMonth}). Hari ini (Tanggal 17) & tanggal berlalu aktif. Tanggal esok (18 – {daysInMonth}) otomatis disabled (abu-abu).
                </p>
              </div>
            </div>
          ...
          <div className="flex items-center justify-between gap-3 border-b border-neutral-100 dark:border-neutral-800 pb-3">
            <div>
              <h3 className="text-base font-bold text-neutral-900 dark:text-white flex items-center gap-2">
                ...
                <span>Edit Matriks Cuaca Bulanan / Monthly Weather Matrix</span>
              </h3>
              <p className="text-xs text-neutral-500 dark:text-neutral-400">
                Pilih kondisi C, B, atau H untuk 24 jam per tanggal (1 – {daysInMonth}).
              </p>
            </div>
            <button
          ...
          <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-3xl p-4 overflow-x-auto space-y-4">
            <div className="text-[11px] font-bold text-neutral-500 tracking-wider flex items-center justify-between">
              <span>Input Matriks Cuaca 24 Jam (Tanggal 1 – {daysInMonth})</span>
              <span className="text-[10px] text-neutral-400 dark:text-neutral-500 font-semibold flex items-center gap-1">
                <Lock className="w-3 h-3 text-neutral-400" />
                Tanggal 18 - {daysInMonth} are disabled & locked
              </span>
            </div>
            <table className="w-full text-center border-collapse text-xs min-w-[950px]">
              <thead>
                <tr className="bg-neutral-100 dark:bg-neutral-800 text-neutral-700 dark:text-neutral-300 font-extrabold text-[10px]">
                  <th className="py-2 px-1 border border-neutral-200 dark:border-neutral-700 w-12">Hour</th>
                  {dateNumbers.map((d) => {
                    const isToday = d === currentTodayDate;
                    const isFuture = d > currentTodayDate;

                    return (
                      <th
                        key={d}
                        className={clsx(
                          "py-2 px-0.5 border border-neutral-200 dark:border-neutral-700 transition-colors",
                          isToday && "bg-amber-100 dark:bg-amber-900/60 text-amber-950 dark:text-amber-100 font-black ring-1 ring-amber-400/50",
                          !isToday && !isFuture && "bg-amber-50 dark:bg-amber-950/40 text-amber-900 dark:text-amber-300 font-extrabold",
                          isFuture && "bg-neutral-100 dark:bg-neutral-800/80 text-neutral-400 dark:text-neutral-500 font-semibold"
                        )}
                      >
                        {d}
                        {isToday && <span className="block text-[7px] text-amber-700 dark:text-amber-300 font-black uppercase">Today</span>}
                      </th>
                    );
                  })}
                </tr>
              ...
              <tbody>
                {hourlyMatrix.map((row, idx) => (
                  <tr key={row.hour} className="hover:bg-neutral-50 dark:hover:bg-neutral-800/40">
                    <td className="py-1 px-1 font-mono font-bold text-neutral-800 dark:text-neutral-200 border border-neutral-200 dark:border-neutral-800 text-[10px]">
                      {row.hour}
                    </td>
                    {dateNumbers.map((d) => {
                      const isFuture = d > currentTodayDate;
                      const val = row.days[d - 1];

                      return (
                        <td key={d} className="p-0 border border-neutral-200 dark:border-neutral-800">
                          {isFuture ? (
                            <select
                              disabled
                              className="w-full py-1 text-center font-bold text-[10px] rounded bg-neutral-100 dark:bg-neutral-800/90 text-neutral-400 dark:text-neutral-600 border-none cursor-not-allowed appearance-none"
                            >
                              <option value="-">-</option>
                            </select>
                          ) : (
                            <select
                              value={val}
                              onChange={(e) => handleCellChange(idx, d, e.target.value as DayVal)}
                              className={clsx(
                                "w-full py-1 text-center font-black text-[10px] rounded outline-none cursor-pointer transition-all appearance-none",
                                val === "C" && "bg-amber-100 text-amber-950 dark:bg-amber-900/60 dark:text-amber-200",
                                val === "B" && "bg-slate-200 text-slate-950 dark:bg-slate-800 dark:text-slate-200",
                                val === "H" && "bg-blue-100 text-blue-950 dark:bg-blue-900/60 dark:text-blue-200",
                                val === "-" && "bg-white text-neutral-300"
                              )}
                            >
          ...
          <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-3xl p-5 space-y-2">
            <label className="text-[11px] font-bold text-neutral-500 tracking-wider block">
              Monthly Weather Evaluation & Executive Summary Notes
            </label>
            <textarea
              rows={3}
              value={monthlyNotes}
              ...
              >
                ...
                >
                  <div className="space-y-3">
                      ...
                      <div className="w-[140px] shrink-0 border border-neutral-300 rounded-sm flex flex-col items-center justify-between p-2 text-center bg-neutral-50/50">
                        ...
                        <div className="w-full grid grid-cols-2 gap-x-1 text-[5px] text-neutral-500">
                          ...
                          <span className="text-left font-bold text-neutral-800">
                            {new Date(`${selectedMonth}-01`).toLocaleDateString("en-GB", { month: "short", year: "numeric" }).toUpperCase()}
                          </span>
                    ...
                    <div className="w-full border-y border-neutral-900 py-1.5 my-2">
                      <div className="grid grid-cols-4 text-center">
                        {[
                          { label: "Period Month", value: new Date(`${selectedMonth}-01`).toLocaleDateString("en-GB", { month: "long", year: "numeric" }) },
                          { label: "Total Sunny Hours", value: `${monthTotals.totalSunny} Hours` },
                          { label: "Total Rainy Hours", value: `${monthTotals.totalRainy} Hours` },
                          { label: "Work Efficiency", value: `${((monthTotals.totalSunny / (24 * currentTodayDate)) * 100).toFixed(1)}%` },
                        ].map((cell, i) => (
                    ...
                    {/* 31-DAY MONTHLY WEATHER MATRIX TABLE (HEADERS ONLY SHOW DATE NUMBERS 1..31) */}
                    <div className="w-full border-y border-neutral-900 bg-white overflow-hidden my-2">
                      <div className="bg-neutral-900 text-white font-bold text-[7.5px] py-1 px-2 tracking-wider flex items-center justify-between">
                        <span>Monthly Weather Matrix / Laporan Cuaca Bulanan</span>
                        <span className="text-[6.5px] font-mono text-neutral-300 font-bold bg-neutral-800 px-1.5 py-0.5 rounded">
                          98 38 00
                        </span>
                      </div>
                      <table className="w-full text-center border-collapse table-fixed" style={{ fontSize: "5.5px" }}>
                        <colgroup>
                          <col style={{ width: "5%" }} />
                          {dateNumbers.map((d) => (
                            <col key={d} style={{ width: `${81 / daysInMonth}%` }} />
                          ))}
                          <col style={{ width: "14%" }} />
                        </colgroup>
                        <thead>
                          <tr className="bg-neutral-50 border-b border-neutral-300 font-bold text-neutral-600">
                            <th className="p-0.5 border-r border-neutral-200">Hour</th>
                            {dateNumbers.map((d) => (
                              <th
                                key={d}
                                className={clsx(
                                  "p-0.5 border-r border-neutral-200 text-center font-bold",
                                  d === currentTodayDate && "bg-amber-100 text-amber-950 font-black",
                                  d > currentTodayDate && "bg-neutral-100/60 text-neutral-400"
                                )}
                              >
                                {d}
                              </th>
                        ...
                        <tbody className="divide-y divide-neutral-200">
                          {hourlyMatrix.map((row) => (
                            <tr key={row.hour} className="border-b border-neutral-200">
                              ...
                              {dateNumbers.map((d) => {
                                const isFuture = d > currentTodayDate;
                                const val = isFuture ? "-" : row.days[d - 1];
                                ...
                                return (
                                  <td
                                    key={d}
                                    className={clsx(
                                      "p-0 border-r border-neutral-200 text-center font-bold transition-colors",
                              ...
                              <td className="p-0.5 text-left font-medium text-neutral-700 pl-1 truncate">
                                {row.remarks || "-"}
                              </td>
                            </tr>
                          ))}

                          {/* TOTAL SUMMARY ROWS MATCHING USER SCREENSHOT & WEEKLY PATTERN */}
                          {/* SUNNY / CERAH C */}
                          <tr className="bg-amber-50 font-bold border-t-2 border-neutral-900">
                            <td className="p-0.5 font-bold text-amber-950 border-r border-neutral-200 text-left pl-1">
                              Cerah (C)
                            </td>
                            {monthTotals.sunnyCounts.map((cnt, i) => {
                              const isFuture = i + 1 > currentTodayDate;
                              return (
                            ...
                            <td className="p-0.5 font-bold text-amber-950 text-center">
                              {monthTotals.totalSunny}
                            </td>
                          </tr>

                          {/* CLOUDY / BERAWAN B */}
                          <tr className="bg-slate-100 font-bold border-t border-neutral-300">
                            <td className="p-0.5 font-bold text-slate-950 border-r border-neutral-200 text-left pl-1">
                              Berawan (B)
                            </td>
                            {monthTotals.cloudyCounts.map((cnt, i) => {
                              const isFuture = i + 1 > currentTodayDate;
                              return (
                            ...
                            <td className="p-0.5 font-bold text-slate-950 text-center">
                              {monthTotals.totalCloudy}
                            </td>
                          </tr>

                          {/* RAIN / HUJAN H */}
                          <tr className="bg-blue-50 font-bold border-t border-neutral-300">
                            <td className="p-0.5 font-bold text-blue-950 border-r border-neutral-200 text-left pl-1">
                              Hujan (H)
                            </td>
                            {monthTotals.rainyCounts.map((cnt, i) => {
                              const isFuture = i + 1 > currentTodayDate;
                              return (
                            ...
                            <td className="p-0.5 font-bold text-blue-950 text-center">
                              {monthTotals.totalRainy}
                            </td>
                          </tr>

                          {/* TOTAL */}
                          <tr className="bg-neutral-900 text-white font-bold border-t border-neutral-900">
                            <td className="p-0.5 font-bold border-r border-neutral-700 text-left pl-1">
                              Total
                            </td>
                            {dateNumbers.map((d) => (
                              <td key={d} className="p-0.5 font-bold border-r border-neutral-700">
                                {d > currentTodayDate ? "-" : 24}
                              </td>
                            ))}
                            <td className="p-0.5 font-bold text-amber-300 text-center">
                              {monthTotals.grandTotal}
                            </td>
                          </tr>
                        </tbody>
                      </table>
                    ...
                    {/* MONTHLY SUMMARY EVALUATION BOX */}
                    <div className="w-full border-y border-neutral-900 bg-white overflow-hidden my-2">
                  ...
                  <div className="pt-2 border-t border-neutral-200 flex items-center justify-between text-[7.5px] font-bold text-neutral-400 tracking-wider">
                    <span>Adidaya Studio | Monthly Weather Summary Report</span>
