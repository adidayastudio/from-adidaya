...
====================== */
const NAV_ITEMS = [
  { label: "Overview", path: "/flow/supply", icon: LayoutDashboard },
  { label: "Requests", path: "/flow/supply/requests", icon: Inbox },
  { label: "Purchase Orders", path: "/flow/supply/orders", icon: FileSignature },
  { label: "Vendors", path: "/flow/supply/vendors", icon: Users },
  { label: "Deliveries", path: "/flow/supply/deliveries", icon: Truck },
  { label: "Reports", path: "/flow/supply/reports", icon: BarChart },
  { label: "Settings", path: "/flow/supply/settings", icon: Settings },
];
...
export default function SupplySidebar() {
  ...
  const isActive = (path: string) => {
    ...
    if (path === "/flow/supply") return pathname === "/flow/supply";
    ...
    <>
      ...
      <aside className="w-full h-full hidden lg:flex flex-col pt-0">
        <div className="space-y-0.5">
          <div className="text-[10px] font-bold text-neutral-400/80 uppercase tracking-widest px-3 mb-2 leading-none">Supply Management</div>
          {NAV_ITEMS.map((item) => {
