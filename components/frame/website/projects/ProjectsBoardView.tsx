...
import { Project, ProjectStatus, PROJECT_STATUS_COLORS } from "./types";
import { Plus, ChevronUp, ChevronDown, Calendar, MapPin, MoreVertical } from "lucide-react";
...
// Status columns matching Social Planner groupings but for Projects
const COLUMNS: { id: ProjectStatus[]; label: string; color: string; headerBg: string }[] = [
    { id: ["NOT_STARTED", "TODO"], label: "Backlog", color: "bg-neutral-50", headerBg: "bg-neutral-200" },
    { id: ["WRITING", "IN_REVIEW", "NEED_APPROVAL"], label: "In Progress", color: "bg-orange-50", headerBg: "bg-orange-200" },
...
export default function ProjectsBoardView({ projects, onEditProject }: Props) {
    return (
            ...
            <div className="flex gap-5 min-h-[500px]" style={{ minWidth: "fit-content" }}>
                {COLUMNS.map(col => (
                    <BoardColumn
                        key={col.label}
                        column={col}
                        projects={projects.filter(p => col.id.includes(p.status))}
...
function BoardColumn({
    ...
    column: { id: ProjectStatus[]; label: string; color: string; headerBg: string };
    projects: Project[];
    onEditProject: (project: Project) => void;
}) {
            ...
            <div className={`px-4 py-3 rounded-t-xl ${column.headerBg}`}>
                <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="bg-white/70 text-neutral-600 text-xs px-2 py-0.5 rounded-full font-semibold">{projects.length}</span>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-1">
                    ...
                    <SortButton label="Publish Date" keyVal="publishDate" />
                    ...
                    >
                        ...
                        <div className="aspect-[4/3] bg-neutral-100 relative overflow-hidden">
                            ...
                            <div className="absolute top-3 right-3 flex gap-2">
                                <span className={clsx(
                                    ...
                                    PROJECT_STATUS_COLORS[project.status] || "bg-white/90 text-neutral-500"
                                )}>
                        ...
                        <div className="p-4">
                            <div className="flex items-start justify-between mb-2">
                                <div>
                                    <div className="text-[10px] font-bold text-red-600 uppercase tracking-wider mb-1">{project.category}</div>
                                    <h3 className="text-sm font-bold text-neutral-900 leading-tight group-hover:text-red-600 transition-colors">{project.name}</h3>
                                </div>
                            </div>
