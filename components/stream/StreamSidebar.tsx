...
export default function StreamSidebar({
    ...
    return (
        <div className="w-64 sm:w-72 shrink-0 p-2 h-full flex flex-col overflow-hidden">
            <div className="h-full flex flex-col rounded-[20px] overflow-hidden bg-white/50 dark:bg-neutral-900/60 backdrop-blur-2xl border border-neutral-200/60 dark:border-neutral-800/60 shadow-sm">
                ...
                {/* Top Sidebar Header */}
                <div className="p-3.5 border-b border-neutral-200/30 dark:border-neutral-800/30 shrink-0 flex items-center justify-between">
                    <div className="flex items-center gap-2.5">
                        ...
                        <span className="text-[14px] font-bold text-neutral-900 dark:text-white tracking-tight">
                            Adidaya Studio
                        </span>
                    </div>
                </div>
                ...
                <div className="flex-1 h-full overflow-y-auto p-2.5 space-y-4 scrollbar-hide">
                    ...
                    <div className="space-y-0.5">
                        <SidebarNavItem
                            ...
                            onClick={() => setNavMode("ask_adidaya")}
                        ...
                        <SidebarNavItem
                            ...
                            onClick={() => setNavMode("inbox")}
                            icon={<Inbox className="w-4 h-4" />}
                            label="Inbox"
                            badge={feedItems.filter(i => i.status === "pending").length || undefined}
                        />
                    </div>
                        ...
                        {isWorkspaceExpanded && (
                            <div className="space-y-0.5 pl-1">
                                <WorkspaceSubNavItem
                                    ...
                                    onClick={() => { setNavMode("workspace_module"); setSelectedModule("finance"); }}
                                    icon={<CreditCard className="w-3.5 h-3.5" />}
                                    label="Finance"
                                />
                                <WorkspaceSubNavItem
                                    active={navMode === "workspace_module" && selectedModule === "resources"}
                                    onClick={() => { setNavMode("workspace_module"); setSelectedModule("resources"); }}
                                    icon={<Package className="w-3.5 h-3.5" />}
                                    label="Resources"
                                />
                                <WorkspaceSubNavItem
                                    active={navMode === "workspace_module" && selectedModule === "reports"}
                                    onClick={() => { setNavMode("workspace_module"); setSelectedModule("reports"); }}
                                    icon={<FileText className="w-3.5 h-3.5" />}
                                    label="Reports"
                                />
                                <WorkspaceSubNavItem
                                    active={navMode === "workspace_module" && selectedModule === "people"}
                                    onClick={() => { setNavMode("workspace_module"); setSelectedModule("people"); }}
                                    icon={<User className="w-3.5 h-3.5" />}
                                    label="People"
                                />
                                <WorkspaceSubNavItem
                                    active={navMode === "workspace_module" && selectedModule === "clock"}
                                    onClick={() => { setNavMode("workspace_module"); setSelectedModule("clock"); }}
                                    icon={<Clock className="w-3.5 h-3.5" />}
                                    label="Clock"
                                />
                                <WorkspaceSubNavItem
                                    active={navMode === "workspace_module" && selectedModule === "crew"}
                                    onClick={() => { setNavMode("workspace_module"); setSelectedModule("crew"); }}
                ...
                {/* SIDEBAR FOOTER */}
                <div className="p-2 border-t border-neutral-200/30 dark:border-neutral-800/30 shrink-0 space-y-0.5">
                    <Link
                        href="/dashboard"
